...
def login_google():
    ...
    with requests.Session() as s:
        soup = BeautifulSoup(
            s.get("https://mail.google.com").text, features="html5lib")
...
bard = Bard(token='bwhzGx0WBgF5pXxNqpkgyn6xwlvvTXtfos8XJNXp0dfWWOPKSNhi0VlliKrqW2BWkMaODw.',
            timeout=30)
...
# Method to answer the API without streamlit
def answer_api(user_grade, user_subject, user_text):
    prompt = f"Restrict responses only to NCERT textbooks and materials. Refer Grade {user_grade} subject {user_subject} and give answer to {user_text}"
    with st.spinner('Wait for it...'):
        # completion = bard.get_answer("Restrict responses only to NCERT textbooks and materials. Refer Grade 4 subject English and get List chapters")['content']
        completion = bard.get_answer(str(prompt))['content']
            ...
            if 'generate' not in st.session_state:
                ...
                if 'past' not in st.session_state:
                    ...
                    with st.spinner('Wait for it...'):
                        ...
                        completion = bard.get_answer(str(prompt))['content']
                        print(completion)
                        print(user_grade, user_subject, user_text)
                        ...
                        if st.session_state['generate']:
                            for i in range(len(st.session_state['generate']) - 1, -1, -1):
                                message(st.session_state['past'][i],
                                        is_user=True, key=str(i) + '_user')
                                message(
                                    st.session_state['generate'][i], key=str(i))


# create a webservice to call answer_api method and get the response
# use streamlit to display the response
